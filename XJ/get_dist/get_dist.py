...
# get cor_SAC
sacfile_path = '/home/zhangh/Data/cor_pws'
sacfile_list = os.listdir(sacfile_path)
sac_num = 0

dists, sta1s, sta2s = [], [], []
for sacfile in sacfile_list:
    part = sacfile.split('.')[0].split('_')
    if len(part) == 3:
        sta1 = part[1]
        sta2 = part[2]
    elif len(part) == 4:
    ...
    # if (sta1[0] in ['A','C','J'] or (sta1[0]=='D' and sta1 != 'DOC')) and (sta2[0] in ['A','C','J'] or (sta2[0]=='D' and sta2 != 'DOC')):
    if (sta1[0]=='B' or sta1=='KMI' or sta1=='DOC') and (sta2[0]=='B' or sta2=='KMI' or sta2=='DOC'):
        sac_num = sac_num +1
        p1 = st_names.index(sta1)
        p2 = st_names.index(sta2)
        stlo1, stla1, stlo2, stla2 = st_los[p1], st_las[p1], st_los[p2], st_las[p2]
        client = Client()
        result = client.distaz(stalat=stla1, stalon=stlo1, evtlat=stla2, evtlon=stlo2)
